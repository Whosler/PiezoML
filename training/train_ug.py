...
class GCN(torch.nn.Module):
    def __init__(self):
        ...
        self.bn1 = torch.nn.BatchNorm1d(256)  # BatchNorm for first layer
        self.conv2 = GCNConv(in_channels=256, out_channels=128)
        self.bn2 = torch.nn.BatchNorm1d(128)  # BatchNorm for second layer
        self.conv3 = GCNConv(in_channels=128, out_channels=64)
        self.bn3 = torch.nn.BatchNorm1d(64)   # BatchNorm for third layer
        self.conv4 = GCNConv(in_channels=64, out_channels=32)  # New GCN layer
        self.bn4 = torch.nn.BatchNorm1d(32)   # BatchNorm for fourth layer
    ...
    def forward(self, data):
        ...
        x = self.conv4(x, edge_index)  # Apply new GCN layer
...
with open('dij_data_list_0.1.pkl', 'rb') as f:
...
torch.save(best_model_state, 'best_dij_model_10.pth')

# Save predictions to CSV
with open('dij_median_predictions_10.csv', 'w', newline='') as csvfile:
...
plt.title('DIJ Weightless GCN Receiver Operating Characteristic')
plt.legend(loc="lower right")
plt.savefig('dij_roc_auc_curve_10.png')
plt.show()
